...
from .image_handler import (
    ...
    VisionConfig,
    count_image_blocks,
    init_vision_concurrency,
    run_ocr_pipeline,
    sanitize_image_payload_for_trace,
    warm_up_vision_backend,
)
from .logging import (
    LOG,
    TerminalSpinner,
...
def maybe_warm_up_vision(config: GatewayConfig) -> GatewayConfig:
    ...
    LOG.info(
        "vision warm-up starting backend=%s model=%s",
        primary.backend,
        primary.model,
    )
    try:
        ...
        if fallback_backend:
            LOG.warning(
                "vision warm-up failed backend=%s reason=%s; trying fallback=%s",
                primary.backend,
                exc,
                fallback_backend,
            )
            fallback_config = replace(
                config,
                vision_backend=fallback_backend,
                vision_fallback_backend="",
            )
            try:
                warm_up_vision_backend(vision_config_from_gateway(fallback_config))
                LOG.warning(
            ...
            LOG.warning(
                "vision warm-up failed backend=%s reason=%s",
                primary.backend,
                exc,
            )
...
class DeepSeekGatewayHandler(BaseHTTPRequestHandler):
    ...
    def traffic_controller(self) -> TrafficController:
        return getattr(
            self.server,
            "traffic_controller",
            TrafficController(max_inflight=0, queue_timeout_seconds=0),
        )

    @property
    def ocr_cache(self) -> OcrCache | None:
        return getattr(self.server, "ocr_cache", None)
    ...
    def do_POST(self) -> None:
        ...
        image_count = count_image_blocks(payload.get("messages", []))

        trace_payload = (
            sanitize_image_payload_for_trace(payload) if image_count > 0 else payload
        )
        if trace is not None:
            trace.record_cursor_body(trace_payload)

        if self.config.verbose:
            log_json("cursor request body", trace_payload)

        log_cursor_request(payload, self.config)

        if image_count > 0:
            if self.config.image_handling == "reject":
                LOG.warning(
                    "rejected request path=%s status=400 reason=image_input image_count=%s",
                    request_path,
                    image_count,
                )
                self._send_json(
                    ...
                    {
                        "error": {
                            "message": str(ImageInputRejected(image_count)),
                ...
                self._finish_trace(
                    trace, "rejected", http_status=400, reason="image_input"
                )
                return
            LOG.info(
                "image detected image_handling=%s image_count=%s request_path=%s",
                self.config.image_handling,
                image_count,
                request_path,
            )

        # --- OCR pipeline ---
        if image_count > 0 and self.config.image_handling == "ocr":
            try:
                vision = vision_config_from_gateway(self.config)
                security = SecurityConfig(
                    max_bytes=self.config.image_max_bytes,
                    max_count=self.config.image_max_count,
                    max_base64_bytes=self.config.image_max_base64_bytes,
                    allow_local_urls=self.config.image_allow_local_urls,
                )
                converted = run_ocr_pipeline(payload, vision, self.ocr_cache, security)
                LOG.info(
                    "ocr complete converted=%s request_path=%s",
                    converted,
                    request_path,
                )
            except (OcrError, ImageSecurityViolation, ImageTooLarge) as exc:
                LOG.warning("ocr failed request_path=%s reason=%s", request_path, exc)
                self._send_json(
                    502,
                    {"error": {"message": str(exc)}},
                    trace=trace,
                )
                self._finish_trace(trace, "ocr_error", http_status=502)
                return
        ...
        if (
            ...
            self._send_json(
                409,
                {
                    "error": {
                        "message": (
                            "deepseek-cursor-gateway is running in strict "
                            ...
                            "`--missing-reasoning-strategy recover`, so the gateway "
        ...
        if self.config.verbose:
            log_json(
                ...
                sanitize_image_payload_for_trace(prepared.payload),
            )
        ...
        try:
            ...
            with self.traffic_controller.open_upstream(
                lambda req=request, timeout=self.config.request_timeout: urlopen(
                    req, timeout=timeout
                )
            ) as response:
                try:
                    with response:
                        ...
                        if prepared.payload.get("stream"):
                            sent_response = self._gateway_streaming_response(
                                response,
                                prepared.original_model,
                            ...
                            sent_response = self._gateway_regular_response(
                                response,
                                prepared.original_model,
                                prepared.payload["messages"],
                                prepared.cache_namespace,
            ...
            spinner.stop()
            LOG.warning(
                "upstream slot unavailable queue_timeout=%s request_path=%s",
                exc.timeout_seconds,
                request_path,
            )
            self._send_json(
    ...
    def _gateway_regular_response(
        ...
        if not sent_headers:
            return GatewayResponseResult(False, usage)
        sent = self._write_to_client(body, "sending upstream response body")
        return GatewayResponseResult(sent, usage)

    def _gateway_streaming_response(
        self,
        response: Any,
        original_model: str,
        request_messages: list[dict[str, Any]],
        ...
        if not sent_headers:
            return GatewayResponseResult(False)
        self.close_connection = True
        ...
        try:
            ...
            # would discard any reasoning the gateway received but never cached.
            if not finalized:
                if self.config.verbose:
                    log_json(
        ...
        return GatewayResponseResult(True, usage)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the local DeepSeek Cursor gateway"
    )
    parser.add_argument(
        "--config",
        dest="config_path",
    ...
    parser.add_argument(
        "--upstream-max-inflight",
        type=int,
        help="Maximum concurrent upstream DeepSeek requests, default 2. 0 = disabled.",
    )
    parser.add_argument(
        "--upstream-queue-timeout-seconds",
        type=float,
        help="Maximum seconds to wait for an upstream slot, default 300",
    )
    parser.add_argument(
        "--upstream-retry",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Enable retry on transient upstream errors (429, 5xx)",
    )
    parser.add_argument(
        "--upstream-retry-max-attempts",
        type=int,
        help="Maximum total upstream attempts, default 3",
    )
    parser.add_argument(
        "--upstream-retry-base-delay-seconds",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        help="Jitter added to retry delay, default 1",
    )
    parser.add_argument(
        "--upstream-respect-retry-after",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Honor Retry-After header from upstream",
    )
    parser.add_argument(
        "--upstream-cooldown-on-429",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Apply process-wide cooldown after 429 responses",
    )
    parser.add_argument(
        "--image-handling",
        choices=["strip", "reject", "ocr"],
        help="How to handle image input: strip (placeholder), reject (error), or ocr (vision model)",
    )
    parser.add_argument(
        "--vision-backend",
        choices=["openai_compatible", "gemini", "tesseract"],
        help="Vision backend for OCR mode, default openai_compatible",
    )
    parser.add_argument(
        "--vision-base-url",
        help="Base URL for the vision API",
    )
    parser.add_argument(
        "--vision-model",
    ...
    parser.add_argument(
        "--vision-api-key",
        help="API key for the vision backend",
    )
    parser.add_argument(
        "--vision-timeout",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Maximum concurrent vision API calls, default 0 (unlimited). "
            "Use 1 or 2 for local LLMs with limited parallel capacity."
        ),
    )
    parser.add_argument(
        "--vision-warmup",
        ...
        help=(
            "Warm up remote vision backends on startup: off, warn, or require. "
            "Default warn."
        ),
    )
    parser.add_argument(
        "--vision-fallback-backend",
        choices=["tesseract", "none", "off"],
        help="Fallback OCR backend when the primary vision backend fails",
    )
    parser.add_argument(
        "--tesseract-lang",
        metavar="LANG",
        help="Tesseract language code(s), e.g. 'eng' or 'eng+chi_sim'",
    )
    parser.add_argument(
        "--request-start-rate-limit",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Enable optional request-start rate smoothing (default: disabled)",
    )
    parser.add_argument(
        "--request-start-rate-per-minute",
        type=float,
        help="Max requests per minute for the start-rate smoother",
    )
    parser.add_argument(
        "--request-start-burst",
        type=int,
        help="Burst capacity for the start-rate smoother",
    )
    return parser
...
def recovery_notice_chunk(
    ...
    return {
        "id": "chatcmpl-deepseek-cursor-gateway-recovery",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        config = GatewayConfig.from_file(config_path=args.config_path)
    except ValueError as exc:
    ...
    if args.upstream_max_inflight is not None:
        updates["upstream_max_inflight"] = args.upstream_max_inflight
    if args.upstream_queue_timeout_seconds is not None:
        updates["upstream_queue_timeout_seconds"] = args.upstream_queue_timeout_seconds
    if args.upstream_retry is not None:
        updates["upstream_retry_enabled"] = args.upstream_retry
    if args.upstream_retry_max_attempts is not None:
        updates["upstream_retry_max_attempts"] = args.upstream_retry_max_attempts
    if args.upstream_retry_base_delay_seconds is not None:
        updates["upstream_retry_base_delay_seconds"] = (
            args.upstream_retry_base_delay_seconds
        )
    ...
    if args.upstream_retry_jitter_seconds is not None:
        updates["upstream_retry_jitter_seconds"] = args.upstream_retry_jitter_seconds
    if args.upstream_respect_retry_after is not None:
        updates["upstream_respect_retry_after"] = args.upstream_respect_retry_after
    if args.upstream_cooldown_on_429 is not None:
        updates["upstream_cooldown_on_429"] = args.upstream_cooldown_on_429
    if args.image_handling is not None:
        updates["image_handling"] = args.image_handling
    if args.vision_backend is not None:
        updates["vision_backend"] = args.vision_backend
    if args.vision_base_url is not None:
        updates["vision_base_url"] = (
            str(args.vision_base_url) if args.vision_base_url else None
        )
    if args.vision_model is not None:
        updates["vision_model"] = args.vision_model
    if args.vision_api_key is not None:
        updates["vision_api_key"] = str(args.vision_api_key)
    if args.vision_timeout is not None:
        updates["vision_timeout"] = args.vision_timeout
    if args.vision_concurrency is not None:
        updates["vision_concurrency"] = args.vision_concurrency
    if args.vision_warmup is not None:
        updates["vision_warmup"] = args.vision_warmup
    if args.vision_fallback_backend is not None:
        updates["vision_fallback_backend"] = (
            ""
            if args.vision_fallback_backend in {"none", "off"}
            else str(args.vision_fallback_backend)
        )
    if args.tesseract_lang is not None:
        updates["tesseract_lang"] = str(args.tesseract_lang)
    if args.request_start_rate_limit is not None:
        updates["request_start_rate_limit_enabled"] = args.request_start_rate_limit
    if args.request_start_rate_per_minute is not None:
        updates["request_start_rate_per_minute"] = args.request_start_rate_per_minute
    if args.request_start_burst is not None:
        updates["request_start_burst"] = args.request_start_burst
    if updates:
    ...
    if config.image_handling == "ocr":
        ocr_cache = OcrCache(
            OcrCacheConfig(
                path=config.image_ocr_cache_path,
                max_age_seconds=config.image_ocr_cache_max_age_seconds,
                max_rows=config.image_ocr_cache_max_rows,
            )
        )
    ...
    if config.trace_dir is not None:
        try:
            ...
            if ocr_cache is not None:
                ocr_cache.close()
            return 2
    server = DeepSeekGatewayServer((config.host, config.port), DeepSeekGatewayHandler)
    ...
    if (
        ...
        and config.request_start_rate_per_minute > 0
        and config.request_start_burst > 0
    ):
        smoother = RequestStartSmoother(
            rate_per_minute=config.request_start_rate_per_minute,
            burst=config.request_start_burst,
        )

    server.traffic_controller = TrafficController(
        max_inflight=config.upstream_max_inflight,
        queue_timeout_seconds=config.upstream_queue_timeout_seconds,
        retry_config=RetryConfig(
            enabled=config.upstream_retry_enabled,
            max_attempts=config.upstream_retry_max_attempts,
            base_delay_seconds=config.upstream_retry_base_delay_seconds,
            max_delay_seconds=config.upstream_retry_max_delay_seconds,
            jitter_seconds=config.upstream_retry_jitter_seconds,
            respect_retry_after=config.upstream_respect_retry_after,
            cooldown_on_429=config.upstream_cooldown_on_429,
        ),
        smoother=smoother,
    )

    server.ocr_cache = ocr_cache
    ...
    if config.image_handling == "ocr":
        LOG.info(
            "vision_backend: %s (fallback: %s)",
            config.vision_backend,
            config.vision_fallback_backend or "none",
        )
    try:
