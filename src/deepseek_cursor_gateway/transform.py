...
RECOVERY_NOTICE_TEXT = "[deepseek-cursor-gateway] Refreshed reasoning_content history."
RECOVERY_NOTICE_CONTENT = f"{RECOVERY_NOTICE_TEXT}\n\n"
RECOVERY_SYSTEM_CONTENT = (
    "deepseek-cursor-gateway recovered this request because older DeepSeek "
    ...
    if isinstance(content, list):
        ...
        for item in content:
            ...
            if item_type in {"text", "input_text"} and isinstance(text, str):
                ...
                parts.append(f"[{item_type} omitted by DeepSeek text gateway]")
...
def strip_recovery_notice_for_upstream(
    ...
    """Cursor echoes the gateway's recovery notice back to us in later turns.
    The notice serves as a boundary marker for the gateway, but DeepSeek must
    not see gateway-generated prose. Return a copy with assistant prefixes
...
def upstream_model_for(original_model: str, config: GatewayConfig) -> str:
