...
through the gateway (captures real request flow on disk)."""
...
class TraceWriterUnitTests(unittest.TestCase):
    ...
    def test_image_data_is_redacted_from_recorded_cursor_body(self) -> None:
        with TemporaryDirectory() as temp_dir:
            writer = TraceWriter(temp_dir)
            trace = writer.start_request(
                method="POST",
            ...
            self.assertIn("[image_url omitted from trace]", serialized)

    def test_ocr_summary_is_redacted_from_recorded_transform(self) -> None:
        with TemporaryDirectory() as temp_dir:
            writer = TraceWriter(temp_dir)
            trace = writer.start_request(
                method="POST",
            ...
            trace.record_transform(
                SimpleNamespace(
                    ...
                    record_response_scope=None,
                    record_response_contexts=[],
                    continued_recovery_boundary=None,
            ...
            serialized = trace.path.read_text(encoding="utf-8")
            self.assertNotIn("SECRET OCR TEXT", serialized)
            self.assertIn("[OCR summary omitted from trace]", serialized)


# ---------------------------------------------------------------------------
# Integration: trace writer attached to a running gateway.
# ---------------------------------------------------------------------------
...
class TraceIntegrationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        gateway.trace_writer = self.writer
    ...
    def test_traces_unsupported_post_path_with_body(self) -> None:
        request = Request(
            f"{self.gateway.url}/v1/summarize",
            data=json.dumps(
                {
                    "model": "gpt-4o-mini",
