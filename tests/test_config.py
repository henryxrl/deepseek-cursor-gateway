...
from deepseek_cursor_gateway.config import (
...
class ConfigTests(unittest.TestCase):
    def test_default_paths_live_in_visible_user_app_directory(self) -> None:
        ...
        with patch("deepseek_cursor_gateway.config.Path.home", return_value=home):
            self.assertEqual(
                default_config_path(), home / ".deepseek-cursor-gateway" / "config.yaml"
            )
            self.assertEqual(
                default_reasoning_content_path(),
                home / ".deepseek-cursor-gateway" / "reasoning_content.sqlite3",
            )
            self.assertEqual(
                GatewayConfig().reasoning_content_path,
                home / ".deepseek-cursor-gateway" / "reasoning_content.sqlite3",
            )
            self.assertEqual(GatewayConfig().ngrok, DEFAULT_NGROK)
            self.assertIsNone(GatewayConfig().ngrok_url)
            self.assertEqual(
                GatewayConfig().collapsible_reasoning,
                DEFAULT_COLLAPSIBLE_REASONING,
            )
            self.assertIsNone(GatewayConfig().trace_dir)
    ...
    def test_missing_default_config_file_is_populated(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            with patch("deepseek_cursor_gateway.config.Path.home", return_value=home):
                config = GatewayConfig.from_file(config_path=None)
    ...
    def test_missing_explicit_config_file_is_not_populated(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            config = GatewayConfig.from_file(config_path=config_path)
    ...
    def test_loads_config_from_user_yaml_file(self) -> None:
        ...
        self.assertEqual(config.vision_concurrency, 3)
        self.assertEqual(config.vision_warmup, "require")
        self.assertEqual(config.vision_fallback_backend, "tesseract")
    ...
    def test_invalid_yaml_config_raises_value_error(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(ValueError):
                GatewayConfig.from_file(config_path=config_path)
    ...
    def test_process_environment_does_not_override_config(self) -> None:
        ...
        with patch.dict(
            "os.environ",
            {
                ...
                "DEEPSEEK_CURSOR_GATEWAY_CONFIG_PATH": "/ignored.yaml",
            },
            clear=True,
        ):
            config = GatewayConfig.from_file(config_path=config_path)
