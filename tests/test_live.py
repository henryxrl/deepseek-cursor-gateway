...
class GatewayFixture:
    def __init__(self) -> None:
        ...
        server = DeepSeekGatewayServer(("127.0.0.1", 0), DeepSeekGatewayHandler)
        server.config = GatewayConfig(
...
class LiveDeepSeekGatewayTests(unittest.TestCase):
    def test_gateway_repairs_real_deepseek_tool_call_history(self) -> None:
        api_key = os.environ["LIVE_DEEPSEEK_KEY"]
        gateway = GatewayFixture().start()
        try:
            ...
            gateway_status, second_response = post_json(
                gateway.url,
                missing_reasoning_payload,
                api_key=api_key,
            )
            self.assertEqual(gateway_status, 200, second_response.get("error"))
