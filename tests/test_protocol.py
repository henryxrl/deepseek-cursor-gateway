...
Each test boots the gateway in-process against a small fake upstream and walks
...
gateway is protocol-compliant, every turn succeeds; if not, the upstream
short-circuits and the test fails fast.

This file is the ground truth for "does the gateway speak DeepSeek correctly?"
"""
...
from deepseek_cursor_gateway.server import DeepSeekGatewayHandler, DeepSeekGatewayServer
...
class _StrictUpstreamCase(unittest.TestCase):
    """Common setup: strict fake DeepSeek + gateway."""
...
class CanonicalLoopTests(_StrictUpstreamCase):
    def test_canonical_four_turn_tool_call_loop(self) -> None:
        """Cursor strips reasoning_content from history; the gateway must
            ...
            f"{self.gateway.url}/v1/chat/completions",
            {
                ...
                "messages": [
                    {"role": "user", "content": "ping"},
                    {
                        "role": "assistant",
                        "content": "",
                        "reasoning_content": "Should be discarded by gateway.",
                        "tool_calls": [
                            {
                                "id": CALL_ID_1,
                                "type": "function",
...
class RecoveryTests(_StrictUpstreamCase):
    def test_cold_cache_recovers_to_latest_user_with_notice(self) -> None:
        """Stale tool history with no cached reasoning: gateway keeps only
        ...
        self.assertIn(
            "[deepseek-cursor-gateway] Refreshed reasoning",
            response["choices"][0]["message"]["content"],
        )

    def test_recovery_notice_is_stripped_before_upstream_replay(self) -> None:
        """When Cursor echoes a previous response (which carried the gateway's
        recovery notice) back as assistant content, the notice serves as a
        boundary marker for the gateway but must not be replayed upstream as
        ...
        for message in sent["messages"]:
            if message.get("role") != "assistant":
                continue
            self.assertNotIn("deepseek-cursor-gateway", message.get("content", ""))
    ...
    def test_recover_mode_does_not_short_circuit_with_409(self) -> None:
        ...
        so `missing_indexes` stays populated. The gateway must NOT 409 in
        ...
        # The point of this test is the gateway did NOT pre-empt with 409.
...
class _StreamingThenJsonHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        # Non-streaming follow-up: enforce that gateway patched the prior
...
class StreamingThenNonStreamingTests(unittest.TestCase):
    ...
    def test_streamed_reasoning_is_replayed_in_next_non_streaming_request(
        ...
        POST. The gateway must repair reasoning_content from the streaming
...
class ConcurrentThreadTests(unittest.TestCase):
    ...
    def test_two_interleaved_threads_do_not_leak_reasoning(self) -> None:
        """Tool-call IDs are reused across both threads. The gateway must
        ...
        status, first_a = _post(f"{self.gateway.url}/v1/chat/completions", first("A"))
        self.assertEqual(status, 200)
        status, first_b = _post(f"{self.gateway.url}/v1/chat/completions", first("B"))
        self.assertEqual(status, 200)
        status, _ = _post(
            f"{self.gateway.url}/v1/chat/completions",
...
class _SlowToolStreamHandler(BaseHTTPRequestHandler):
    ...
    Lets us verify the gateway stores reasoning at finish_reason=tool_calls,
    not at [DONE]."""
